# GPT Actions Testing Prompts

## Purpose
Copy-paste prompts for gpt actions testing prompts in AI Citation Auditor workflows.

## When to use it
- Use when you need a reusable prompt for happy-path tests, missing parameter tests, invalid auth tests, empty result tests, rate-limit simulation, private-data boundary tests, and user-facing error review.
...
### Prompt 1
```text
Audit the following gpt actions testing for happy-path tests, missing parameter tests, invalid auth tests, empty result tests, rate-limit simulation, private-data boundary tests, and user-facing error review. Return a table with issue, severity, evidence, fix, and next step. Do not guarantee rankings, citations, funding, approvals, or financial outcomes.
```

### Prompt 2
```text
Rewrite only the risky or unclear parts of the gpt actions testing. Preserve intent, keep the copy useful, and remove absolute or unsupported claims.
```

### Prompt 3
```text
Break the task into sections and evaluate each section separately for happy-path tests, missing parameter tests, invalid auth tests, empty result tests, rate-limit simulation, private-data boundary tests, and user-facing error review. Quote the exact line or heading that needs attention.
```

### Prompt 4
```text
Create a concise checklist for happy-path tests, missing parameter tests, invalid auth tests, empty result tests, rate-limit simulation, private-data boundary tests, and user-facing error review that an operator can run before publishing or shipping.
```

### Prompt 5
```text
Return a structured summary for gpt actions testing with what is working, what is weak, what is risky, and what should happen next.
```
