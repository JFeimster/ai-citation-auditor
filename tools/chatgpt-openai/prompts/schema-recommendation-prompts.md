...
## Purpose
Copy-paste prompts for schema recommendation prompts in AI Citation Auditor workflows.

## When to use it
- Use when you need a reusable prompt for FAQPage, Article, SoftwareApplication, Organization, BreadcrumbList, and HowTo rejection rules.
...
### Prompt 1
```text
Audit the following schema recommendation for FAQPage, Article, SoftwareApplication, Organization, BreadcrumbList, and HowTo rejection rules. Return a table with issue, severity, evidence, fix, and next step. Do not guarantee rankings, citations, funding, approvals, or financial outcomes.
```

### Prompt 2
```text
Rewrite only the risky or unclear parts of the schema recommendation. Preserve intent, keep the copy useful, and remove absolute or unsupported claims.
```

### Prompt 3
```text
Break the task into sections and evaluate each section separately for FAQPage, Article, SoftwareApplication, Organization, BreadcrumbList, and HowTo rejection rules. Quote the exact line or heading that needs attention.
```

### Prompt 4
```text
Create a concise checklist for FAQPage, Article, SoftwareApplication, Organization, BreadcrumbList, and HowTo rejection rules that an operator can run before publishing or shipping.
```

### Prompt 5
```text
Return a structured summary for schema recommendation with what is working, what is weak, what is risky, and what should happen next.
```
